...
export default function LanguageSelector({ value, onLangChange }) {
    return (
            ...
            >
                {LANG_OPTIONS.map((opt) => (
                    <button
                        ...
                        className={
                            opt === value
                                ? "px-4 py-2 text-xs font-bold rounded-full bg-primary text-white shadow-md shadow-primary/20 transition-all cursor-pointer"
                                : "px-4 py-2 text-xs font-bold rounded-full text-slate-600 hover:bg-white border border-transparent transition-all cursor-pointer"
                        }
                    >
                        {opt}
                    </button>
